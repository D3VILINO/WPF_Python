from sympy import Eq, Symbol, parse_expr, solve, lambdify, simplify #type: ignore[import-untyped]
import re
from tkinter import END, Entry, Label

unique_vars:list[Symbol] = []
f1:str = ""
solution:list = []
...
def submitted_entry(entry_function:str, output_label:Label) -> None:
  ...
  solve_for:str = str(unique_vars[0])

  if valid:
    solution = my_solve(unique_vars[0])
    print(solution)
    text:str = solve_for + " = " + solution[0] + "\n"
    if len(solution) > 1:
      text = ""
      subscripted_char = ("\u2080", "\u2081", "\u2082", "\u2083", "\u2084", "\u2085", "\u2086", "\u2087", "\u2088", "\u2089")
      for i in range(0, len(solution)):
        text += solve_for + subscripted_char[i+1] + " = " + solution[i] + "\n"
    output_label.configure(text=text)
  else:
    output_label.configure(text=info)

def my_solve(solve_for:Symbol) -> list:
  global f1
  f1_left, f1_right = map(parse_expr, f1.split("="))
  eq:Eq = Eq(f1_left, f1_right) # type: ignore
...
def make_parsable(func:str) -> str:
  global unique_vars
  operators:set[str] =  {"+", "-", "*", "/", "=", "^", "(", ")", "sqrt"}
  numbers:set[str] = {str(i) for i in range(0,10)}

  func = func.replace(" ", "")
  akk = sorted(list(set(func).difference(operators).difference(numbers)))
  for i in range(len(akk)):
    unique_vars[i] = Symbol(akk[i])

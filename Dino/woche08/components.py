...
def create_body(frame:Frame, header_label:Label) -> None:
  frame_body:Frame = Frame(frame)
  frame_body.pack()

  entry:Entry = Entry(frame_body, text="New Database", font=("Helvetica", 16))
  entry.grid(row=0, column=0, columnspan=3, padx=5, pady=20)

  button:Button = Button(frame_body, text="New Database", padx=5, font=("Helvetica", 16), command=partial(get_database, entry, header_label, "create"))
  button.grid(row=1, column=0, padx=5)
  button = Button(frame_body, text="Open Database", padx=5, font=("Helvetica", 16), command=partial(get_database, entry, header_label, "open"))
